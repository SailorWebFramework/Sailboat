...
final class MockRenderable: Renderable {
    ...
    // Structural call logs (deep indices passed by Renderable+Build / +Reconcile)
    ...
    func addToParent(_ parent: any Renderable) {
        addToParentCalls += 1
        addToParentTargets.append(ObjectIdentifier(parent as AnyObject))
    }

    func insertAfter(_ deepIndex: Int, parent: any Renderable) {
        insertAfterCalls.append(deepIndex)
    }

    func insertBefore(_ deepIndex: Int, parent: any Renderable) {
        insertBeforeCalls.append(deepIndex)
    }
